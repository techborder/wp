...
class temptation_Featured_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'temptation_fp_cat', // Base ID
			__('IH Featured Posts by Category', 'temptation'), // Name
			array( 'description' => __( 'Display your Featured Content, with a Thumbnail.', 'temptation' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Featured Articles', 'temptation' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'temptation' );
		}
		if ( isset( $instance[ 'fcat' ] ) ) {
			$fcat = $instance[ 'fcat' ];
		}
		else {
			$fcat = __( '1', 'temptation' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','temptation' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'temptation' ); ?></label>
		...
		<label for="<?php echo $this->get_field_id( 'fcat' ); ?>"><?php _e( 'Category ID', 'temptation' ); ?></label>
...
add_action( 'widgets_init', 'register_temptation_f_widget' );  
function register_temptation_f_widget() {  
    register_widget( 'Temptation_Featured_Posts' );  
}  
