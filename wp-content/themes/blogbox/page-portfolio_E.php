...
<div id="fullwidth">
	...
	<?php // additional loop via get_posts
		$category_ID = get_cat_ID(sanitize_text_field($blogBox_options['bB_portfolioE_category']));
		global $post,$feature_pic_count;

		$args = array('category'=>$category_ID,'numberposts'=>999);
		$custom_posts = get_posts($args);
		if ($blogBox_options['bB_portfolioE_cols'] == "2") {
			echo '<div class="portfolio_two_column_wrap">';
		} elseif ($blogBox_options['bB_portfolioE_cols'] == "3") {
			echo '<div class="portfolio_three_column_wrap">';
		} elseif ($blogBox_options['bB_portfolioE_cols'] == "4") {
		...
		if ($category_ID !== 0 && $custom_posts){
			...
			foreach($custom_posts as $post) : setup_postdata($post);
				if ($blogBox_options['bB_portfolioE_cols'] == "1") {
		 				...
		 				echo '<div class="portfolio_one_column">';
							...
							echo '<div class="left_col">';
        						echo '<a href="';the_permalink();echo '" title="';the_title_attribute(); echo '" >';
      							...
      							echo '</a>';
      							if($blogBox_options['bB_portfolioE_feature_caption'] == 1) {
      								echo '<div class="display_caption">';
      									echo '<h4>';echo get_post(get_post_thumbnail_id())->post_excerpt;echo '</h4>';
									echo '</div>';
								}
      							if($blogBox_options['bB_portfolioE_feature_description'] == 1) {
				...
				} elseif ($blogBox_options['bB_portfolioE_cols'] == "2") {
			 		...
			 		if (has_post_thumbnail()) {
						echo '<div class="portfolio_two_column">';
							...
							echo '</div>';
	      					...
	      					if($blogBox_options['bB_portfolioE_content'] == 1) {
			...
			echo '<h3>'.esc_html__('Error: no posts or categories are wrong!','blogBox').'</h3>';
			echo '</div>';
		}
	echo '</div>';
