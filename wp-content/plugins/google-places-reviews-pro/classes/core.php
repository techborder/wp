...
if ( ! class_exists( 'GPR_Plugin_Framework' ) ) {
	...
	class GPR_Plugin_Framework {
		...
		function __construct( $file, $args = array() ) {
			...
			$this->item_name = 'Google Places Reviews Pro';

			//Licence Args
			$licence_args = array(
				'plugin_basename'     => GPR_PLUGIN_NAME_PLUGIN, //Name of License Option in DB
				'settings_page'       => 'settings_page_googleplacesreviews', //Name of License Option in DB
				'store_url'           => $this->store_url, //Name of License Option in DB
				'item_name'           => $this->item_name, //Name of License Option in DB
				'licence_key_setting' => 'gpr_licence_setting', //Name of License Option in DB
				'licence_key_option'  => 'edd_gpr_license_key', //Name of License Option in DB
				'licence_key_status'  => 'edd_gpr_license_status', //Name of License Option in DB
			);
			...
			$this->licencing = new GPR_License( $licence_args );
		...
		function edd_sl_wordimpress_updater() {
			...
			// setup the updater
			$edd_updater = new EDD_SL_Plugin_Updater( $this->store_url, GPR_PLUGIN_NAME_PLUGIN, array(
		...
		function enqueue_assets() {
			...
			foreach (
				array(
					'jquery', 'media-upload', 'thickbox', 'farbtastic', 'core-plugin-framework-form', 'core-plugin-fitvids', 'core-plugin-framework'
				) as $script
			) {
