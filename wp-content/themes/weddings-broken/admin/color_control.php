...
class web_dor_color_control_page_class{
	...
	function __construct(){
		...
		$this->shortcolorcontrol = $special_id_for_db."cc";
		...
		$value_of_std[13]=get_theme_mod($this->shortcolorcontrol."_menu_color",'#464646');
	...
	public function web_bussines_customize_register( $wp_customize ) {
	  ...
	  $wp_customize->add_section( 'aaa' , array(
		'title'   => __( 'Schemes', 'web_bussines' ),
		'priority'  => 49,
	  ) );
	  
		foreach($this->options_colorcontrol as $key=>$spec_color)
		{
			if($spec_color['type']=='picker'){
				
				 $wp_customize->add_setting( 'theme_mods_weddings['.$spec_color['id'].']', array(
				  ...
				  $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'theme_mods_weddings['.$spec_color['id'].']', array(
					'label'    => $spec_color['name'],
					'section'  => 'colors',
					'settings' => 'theme_mods_weddings['.$spec_color['id'].']'
				  ) ) );
...
#back h3 a{
	color: <?php echo '#'.$this->negativeColor(wedding_ligthest_brigths($menu_elem_back_color, 10)); ?> !important;
}
...
.children .comment{
	  background-color:#<?php echo wedding_ligthest_brigths($sideb_background_color,37); ?>;
}

.comment-body .reply a,
#cancel-comment-reply-link{
	background-color: #<?php echo wedding_darkest_brigths($text_headers_color,10); ?>;
}
...
.commentlist > li.parent > div{
	border-left: 3px solid #<?php echo wedding_darkest_brigths($text_headers_color,10); ?>;
}

cite{
	color: #<?php echo wedding_darkest_brigths($text_headers_color,20); ?> !important;
}
...
.read_more:hover, a.read_more:hover, .more-link:hover {
      color:<?php echo '#'.wedding_ligthest_brigths($menu_links_color,50); ?> !important;
     background-color: <?php echo '#'.wedding_ligthest_brigths($menu_elem_back_color,15); ?>;
}

#back {
     background-color: <?php echo '#'.wedding_ligthest_brigths($menu_elem_back_color, 10); ?>;
}
...
.phone .top-nav-list li.current-page-item{
    background-color: <?php echo $selected_menu_bgcolor; ?> !important;
}
...
#commentform #submit{
	    background-color: #<?php echo wedding_darkest_brigths($text_headers_color,20); ?>;
		color:#FFF !important;
}
...
#top-nav-list > li ul  li{
		border-bottom: 1px solid #<?php echo wedding_darkest_brigths($text_headers_color,10); ?> ;
}
...
#top-nav-list > li ul{
	...
	border-bottom:2px solid #<?php echo wedding_ligthest_brigths($selected_menu_color,30); ?>;
	width: 100%;
}
...
.top-nav-list.phone  li ul li.has-sub > a, .top-nav-list.phone  li ul li.has-sub > a:link, .top-nav-list.phone  li ul li.has-sub > a:visited{
	background:<?php echo '#'.wedding_ligthest_brigths($menu_elem_back_color,10); ?>  url(<?php echo get_template_directory_uri('template_url'); ?>/images/menu_right.png) right no-repeat;
	background-size: contain !important;
}
...
.phone #top-nav-list > li ul li.current-menu-item > a{
	border-left: 2px solid #<?php echo wedding_ligthest_brigths($menu_color,40); ?> !important;
}
	...
	public function dorado_theme_admin_color_control(){
		...
		<div id="main_color_control_page">			
			<table align="center" width="90%" style="margin-top: 0px; margin-bottom: 23px;border-bottom: rgb(111, 111, 111) solid 2px;">
					<tr>
						<td>
							<h3 style="margin: 0px;font-family:Segoe UI;padding-bottom: 15px;color: rgb(111, 111, 111); font-size:18pt;">Color Control</h3>
						</td>
		  ...
		  <div style="margin: 0 auto;width:90%;padding-bottom:0px; padding-top:0px;">		
			<form method="post" action="themes.php?page=web_dorado_theme&controller=color_control_page">
			
							<div class="optiontitle">
								<h3>Color Scheme</h3>
							</div>
							<div class="optiondescription">
								<p>The theme has different color schemes. Use the drop-down menu below to change the color scheme. Click
									Save to apply the new scheme.</p>
							</div>
							<div class="optioninput1">
								<?php $this->default_theme_select($array_of_colors); ?>
							<div style="clear:both;"></div>
					<?php for($i=0;$i<$count_picker;$i++){	
						$dor_admin_helepr_functions->only_color($pickers[$i]);  /// colors
					}		
					?>
