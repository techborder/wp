...
function woodpecker_optionsframework_add_admin() {
    global $query_string;

    $themename = woodpecker_get_option('of_themename');
    $shortname = woodpecker_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = woodpecker_get_option('of_template');
            woodpecker_reset_options($options, 'optionsframework');
    ...
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'woodpecker_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'woodpecker_load_only');
}

add_action('admin_menu', 'woodpecker_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function woodpecker_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        woodpecker_delete_option('of_options');
    }
}
...
function woodpecker_optionsframework_options_page() {
    $options = woodpecker_get_option('of_template');
    $themename = woodpecker_get_option('of_themename');
    ?>
	 <div class="trail-notify">
        <h1>Get WoodPecker Theme PRO!</h1>
        <p style="font-size:15px; line-height: 20px;">You are using the WoodPecker lite version. Upgrade to pro for extra features like home page slider, video box, fullWidth page templates and many other, multiple color options, premium theme support and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/woodpecker-business-wordpress-theme/" target="blank">Upgrade to WoodPecker PRO theme here >></a>
    </div>
    ...
    <div class="wrap" id="of_container">
        ...
        <form action="" enctype="multipart/form-data" id="ofform">
            <div id="header">
                ...
                <a href="http://www.woodpecker.com" target="_new">
            ...
            $return = woodpecker_optionsframework_machine($options);
            ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = woodpecker_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = woodpecker_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - woodpecker_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'woodpecker_ajax_callback');

function woodpecker_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        woodpecker_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = woodpecker_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            woodpecker_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    woodpecker_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    woodpecker_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            woodpecker_update_option($multicheck_id, 'false');
                        } else {
                            woodpecker_update_option($multicheck_id, 'true');
                        }
                    }
                } elseif ($type == 'color') {
                    woodpecker_update_option($id, woodpecker_validate_hex($new_value));
                    ...
                    woodpecker_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    woodpecker_update_option($id, $border_array);
...
function woodpecker_validate_hex($hex) {
    $hex = trim($hex);
...
function woodpecker_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = woodpecker_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = woodpecker_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= woodpecker_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= woodpecker_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = woodpecker_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }                             
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = woodpecker_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = woodpecker_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = woodpecker_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - woodpecker_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function woodpecker_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = woodpecker_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (woodpecker_get_option($id) != "") {
            $val = woodpecker_get_option($id);
        }
