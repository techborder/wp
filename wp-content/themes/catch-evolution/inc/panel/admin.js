jQuery( document ).ready( function() {
	var active = 0;
	if( jQuery.cookie( 'catchevolution_ad_tab' ) ) {
		active = jQuery.cookie( 'catchevolution_ad_tab' );
		jQuery.cookie( 'catchevolution_ad_tab', null );
	}
	
	var tabs = jQuery( '#catchevolution_ad_tabs' ).tabs( { active: active } );
	
	jQuery( '#wpbody-content form' ).submit( function() {
		var active = tabs.tabs( 'option', 'active' );
		jQuery.cookie( 'catchevolution_ad_tab', active );
	} );
