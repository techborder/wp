...
if ( $related ) {
	...
	//@since 3.0.3, modified by denzel, made string translatable.
	$retval .= '<li>'.__('No related posts found','truethemes_localize').'</li>';}
...
global $wpdb, $post, $table_prefix;
if ($post->ID) {
	$retval = '<div class="related_posts"><h4>'.$title.'</h4><ul class="list list1">';
// Get tags
$tags = wp_get_post_tags($post->ID);
$tagsarray = array();
foreach ($tags as $tag) {
...
if ( $related ) {
    ...
    $retval .= '<li>';
    $retval .= __('No related posts found','truethemes_localize');
    $retval .= '</li>';
	}
