...
function wedding_home_about_us() {
...
function wedding_content_posts_for_home() {
	global $dor_general_settings_page,$dor_home_page,$wp_query,$paged;
// initila home setings variables
foreach ($dor_home_page->options_homepage as $value) 
{
	if(isset($value['id']))
...
<?php
					...
					<?php
					   if($date_enable){ ?>
								 <div class="home-post-date">
									<?php echo wedding_posted_on();?>
								 </div>
						<?php } 
					   if($grab_image)
					   {
			            echo wedding_display_thumbnail(150,150); 
			           }
				       else 
					   {
					    echo wedding_thumbnail(150,150);
				       }
					  if($blog_style) 
                        {
                           the_excerpt();
		    ...
		    <?php }	
function wedding_content_posts() {
global $dor_general_settings_page,$dor_home_page,$wp_query,$paged;
// initila home setings variables
foreach ($dor_home_page->options_homepage as $value) 
{
    ...
    if (get_theme_mod( $value['id'] ) === FALSE)
	{
		...
		$wp_query = new WP_Query();
			...
			<div class="blog" id="blog_posts">
				...
				<?php
					$i=0;
					$wp_query->query('showposts=1&cat='.wedding_remove_last_comma($content_post).'&paged='.$paged.'&order=ASC');
					 if(have_posts()) { 
						...
						<div class="blog-post">
							...
							/*   if($grab_image)
							   {
								echo wedding_display_thumbnail('250','100'); 
							   }
							   else 
							   {
								echo wedding_thumbnail('250','130');
							   }
							   */
							   ?>
			...
			if($categ_checked && $n_of_home_post!=0){
			$content_categories=$wp_query->query('showposts='.($n_of_home_post).'&cat='.wedding_remove_last_comma($content_post_categories).'&paged='.$paged.'&order=ASC');
						 if(!empty($content_categories)){
						 ?>
				<div id="blog" class="blog blog_gal" >
					...
					<div class="blog_gellery">
						...
						while ($wp_query->have_posts()) {
							...
							<div class="blog-post">
								...
								<a class="title_href" href="<?php echo get_permalink() ?>">
								<?php
								//echo wedding_display_thumbnail('100%','100%');
								
								   if($grab_image)
								   {
									echo wedding_display_thumbnail('200','150'); 
								   }
								   else 
								   {
									echo wedding_thumbnail('350','300');
								   }
			...
			*/						?>
			...
			if($cat_checked && $n_of_testimonials!=0){
			$content_post_cats=$wp_query->query('showposts='.($n_of_testimonials).'&cat='.wedding_remove_last_comma($content_post_cats).'&paged='.$paged.'&order=ASC');
						 if(!empty($content_post_cats)){
			
			?>
			<div class="blog blog_testim">
	...
	<li class="blog-post">
		...
		<div class="text">
			...
			<p>
				<?php
					if($blog_style)
						wedding_the_excerpt_max_charlength(150);
					else 
						the_content(); ?>
			</p>
		</div>
...
function wedding_integrate_is_baner(){
global $dor_integration_page;
// initila integration setings variables
foreach ($dor_integration_page->options_integration as $value)
 ...
 if ($integrate_is_baner_enable == 'on')
 { 
	if($integrate_baner_adsens_code <> '')
		echo ( $integrate_baner_adsens_code );
	else { ?>
		<a href="<?php echo esc_url($integrate_baner_click_destination); ?>">
		   <img src="<?php echo esc_url($integrate_baner_image_url); ?>" alt="468 ad"  />
		</a>
<?php } 
 } 
}
function wedding_entry_meta_cont(){
...
?>
 <a  <?php if( $show_facebook_icon=='' || $facebook_url == "" ){ echo "style=\"display:none;\""; } ?> href="<?php if( trim($facebook_url) ) { echo esc_url($facebook_url);} else { echo "javascript:;";}?>"  target="_blank" title="Facebook">
	<img src="<?php  echo get_template_directory_uri('template_url'); ?>/images/fb.jpg" width="45" height="45" alt="" />
 </a>
 <a <?php if( $show_twitter_icon=='' || $twitter_url == ""){ echo "style=\"display:none;\""; } ?> href="<?php if( trim($twitter_url) ){ echo esc_url($twitter_url);} else { echo "javascript:;";}?>" target="_blank" title="Twitter">
	<img src="<?php  echo get_template_directory_uri('template_url'); ?>/images/tw.jpg" width="45" height="45" alt="" />
 </a>
 <a <?php if( $show_rss_icon=='' || $rss_url == "" ) { echo "style=\"display:none;\""; } ?>  href="<?php if( trim($rss_url) ) { echo esc_url($rss_url);} else { echo "javascript:;";}?>" target="_blank" title="Rss">
	<img src="<?php  echo get_template_directory_uri('template_url'); ?>/images/rss.jpg" width="45" height="45" alt="" />
 </a>
<?php
}
function wedding_custom_css(){
global 	$dor_general_settings_page;
foreach ( $dor_general_settings_page->options_generalsettings as $value ) {
	if(isset($value['id'])){
		if ( get_theme_mod( $value['id'] ) === FALSE ) {
                    ...
                    <div id="logo-block">
						<a class="hedar-a-element" href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home">
							...
							<?php 
								...
								elseif($color_scheme == "Theme-1"){?> 
									<h1 id="site-title">
										<span>
											<a style="color: #c7c6c6;" href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home">
											 <?php   echo "<img id=\"site-title\" src=\"".get_template_directory_uri('template_url').'/images/logo1.png'."\" alt=\"logo\">";   ?>
										   </a>
									   </span>
									</h1>
									<div id="description">
										<p id="site-description-p" style="margin: -15px 0px 0px 20px; text-align: right;"><?php bloginfo( 'description' ); ?></p>
									</div>
									<?php
...
function wedding_page_blog(){
global $dor_general_settings_page,$post;
foreach ( $dor_general_settings_page->options_generalsettings as $value ) 
{
    if ( get_theme_mod( $value['id'] ) === FALSE ) 
	{ 
...
<?php } ?>
                    <div class="blog-post blog-page">	
								<?php
								if (!isset($web_business_meta['showthumb']) || $web_business_meta['showthumb']!="on") 
								{
									if($grab_image && !has_post_thumbnail()) 
									{
										echo wedding_display_thumbnail(200,200); 
									}
									else 
									{
									   echo wedding_thumbnail(200,200);
									}
								} ?>
								<a href="<?php the_permalink(); ?>"  class="read_blog"><?php the_title() ?></a>
								<?php
							if(isset($web_business_meta['blogstyle']) && $web_business_meta['blogstyle']=="on") 
								{ ?>								
								   <style>
...
function wedding_slideshow(){
global $dor_home_page,$dor_slider_page;
foreach ($dor_home_page->options_homepage as $value)
 {
	if(isset($value['id'])){
		if (get_theme_mod( $value['id'] ) === FALSE)
...
wedding_slideshow_java_script($wedding_js_parameters);
?>
...
function wedding_top_posts(){
global $dor_home_page,$dor_general_settings_page,$wp_query;
foreach ( $dor_general_settings_page->options_generalsettings as $value ) 
{
    if ( get_theme_mod( $value['id'] ) === FALSE ) 
	{ 
...
foreach ($dor_home_page->options_homepage as $value) 
{
    ...
    if ($hide_top_posts=='' && is_home()) {	
                 $topPosts = 0;	
				$wedding_query = new WP_Query('showposts=3&paged='.$paged.'&cat='.$top_post_categories.'&orderby=date&order=ASC');
		 if(have_posts()) { 
		 ?>
			...
			<div class="container">
				<ul id="top-posts-list">
				 ...
				 <?php
					...
					while ($wedding_query->have_posts()) 
					{	
						$wedding_query->the_post();
						if($topPosts++ == 3)
						{
							break;
						}														
					...
					<li>
							...
							<a href="<?php the_permalink(); ?>">
									<div class="image-block">
										<?php if($grab_image) 
								{
									echo wedding_display_thumbnail('320','210'); 
								}
								else 
								{
								    echo wedding_thumbnail('320','210');
								} ?>
									</div>
								</a>
								<div class="top-posts-texts">
										<h3 class="heading"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
									<span><?php wedding_the_excerpt_max_charlength(80); ?></span>
								</div>
					 ...
					 if(have_posts()) { 
						...
						<div class="blog-post">
							...
							/*   if($grab_image)
							   ...
							   */
							   ?>
								  <h2><a class="title_href" href="<?php echo get_permalink() ?>"><?php the_title(); ?></a></h2>
				...
				<div class="blog_test">
					...
					<div class="blog-post">
						...
						<?php
							 ...
							 if($blog_style) 
								{
								   wedding_the_excerpt_max_charlength(120);
								}
								else 
								{
								   the_content();
...
?>
             <div class="entry">	
							 ...
							 if (!isset($web_business_meta['showthumb'])) 
							{
								if($grab_image) 
								{
									echo wedding_display_thumbnail(220,220); 
								}
								else 
								{
								   echo wedding_thumbnail(220,220);
								}							
							}
...
function wedding_page_services(){
global $dor_general_settings_page;
foreach ( $dor_general_settings_page->options_generalsettings as $value ) 
{
    if ( get_theme_mod( $value['id'] ) === FALSE ) 
...
function wedding_slideshow_java_script($wedding_js_parameters){

	?>
<script>
	var wd_bwg_transition_duration = <?php echo $wedding_js_parameters["wdspeed"];?>;
	// For browsers that does not support transitions.
	function wd_bwg_fallback(current_image_class, next_image_class, direction) {
		wd_bwg_<?php echo $wedding_js_parameters["effect"];?>(current_image_class, next_image_class, direction);
		
	}
	
	function wd_bwg_<?php echo $wedding_js_parameters["effect"];?>(current_image_class, next_image_class, direction) {
	...
	function wd_bwg_change_image(current_key, key, data) {
     ...
     function wd_bwg_popup_resize() {
	   ...
	   //standart chap vor@ voroshvac chi bnav template um
		...
		var str=(<?php echo $wedding_js_parameters["height"];?>/firstsize  );
		//alert(parentWidth+' '+bodyWidth+' '+firstsize);
		   ...
		   jQuery(".wd_bwg_slideshow_image_wrap > div").css({height: ((parentWidth) * str)});
		    $(".wd_bwg_slideshow_title_container > div").css({height: ((parentWidth) * str)});
			
			 // jQuery(".wd_bwg_slideshow_image").css({height: ((parentWidth) * str)});
      ...
      //  jQuery(".wd_bwg_slideshow_description_text").css({display: 'none'});
        ...
        jQuery('.wd_bwg_slideshow_filmstrip').bind(mousewheelevt, function(e) {
          var evt = window.event || e //equalize event object     
          evt = evt.originalEvent ? evt.originalEvent : evt; //convert to originalEvent if possible               
          var delta = evt.detail ? evt.detail*(-40) : evt.wheelDelta //check for detail first, because it is used by Opera and FF
          if (delta > 0) {
            // Scroll up.
            jQuery(".wd_bwg_slideshow_filmstrip_left").trigger("click");
          }
          else {
            // Scroll down.
            jQuery(".wd_bwg_slideshow_filmstrip_right").trigger("click");
          }
        });*/
        // Set filmstrip initial position.
        ...
        function play() {
          ...
          wd_bwg_playInterval = setInterval(function () {
            ...
            /*if (!data[parseInt(jQuery('#wd_bwg_current_image_key').val()) + iterator]) {
              // Wrap around.
              jQuery('#wd_bwg_current_image_key').val((parseInt(jQuery('#wd_bwg_current_image_key').val()) + iterator) % data.length - 1);
            }*/
          ...
          }, '<?php echo $wedding_js_parameters["pausetime"];?>');
		 
        }
		
		var hideslider="<?php echo $wedding_js_parameters["hideslider"];?>"; 
		
		//alert(hideslider);
		
			var pausehover="<?php echo $wedding_js_parameters["pausehover"];?>";
			
			if(pausehover=="true"){
				$("#wd_bwg_slideshow_image_container, .wd_bwg_slideshow_image_container").hover(function(){clearInterval(wd_bwg_playInterval);},function(){play();});
			}
