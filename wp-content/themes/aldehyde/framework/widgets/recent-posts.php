...
class aldehyde_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'aldehyde_rp', // Base ID
			__('Aldehyde Recent Posts with Thumbnails', 'aldehyde'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'aldehyde' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		$title = __('Latest Posts','aldehyde');
		$no_of_posts = 5;
	endif;
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'aldehyde' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'aldehyde' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','aldehyde' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'aldehyde' ); ?></label>
...
add_action( 'widgets_init', 'register_aldehyde_widget' );  
function register_aldehyde_widget() {  
    register_widget( 'aldehyde_Recent_Posts' );  
}  
