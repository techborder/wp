...
require_once( TRUETHEMES_EXTENDED . '/seo-module/seo_module_class.php');
...
function tt_aioseop_mrt_pcolumns($aioseopc) {
    $aioseopc['seotitle'] = __('SEO Title','truethemes_localize');
    $aioseopc['seokeywords'] = __('SEO Keywords','truethemes_localize');
    $aioseopc['seodesc'] = __('SEO Description','truethemes_localize');
    return $aioseopc;
}
		...
		if( function_exists('get_post_types')){		
			...
			foreach ($mrt_aioseop_pts as $mrt_aioseop_pt) {
				if($mrt_aioseop_pt == 'post' || $mrt_aioseop_pt == 'page' || $aioseop_mrt_cpt){
					...
					add_meta_box('aiosp',__('SEO Settings', 'truethemes_localize'),'tt_aiosp_meta',$mrt_aioseop_pt);
				}
			}
		}else{
		
		    //mod by tt, change meta box title to SEO Module
			add_meta_box('aiosp',__('SEO Settings', 'truethemes_localize'),'tt_aiosp_meta','post');
			add_meta_box('aiosp',__('SEO Settings', 'truethemes_localize'),'tt_aiosp_meta','page');
		}
...
<!--	suppress by tt	
		<a target="__blank" href="http://semperfiwebdesign.com/forum/"><?php _e('Click here for Support', 'truethemes_localize') ?></a>
...
<!--Note by TT: The codes below are for constructing html of post meta box!-->
		...
		<td><input value="<?php echo $title ?>" type="text" name="aiosp_title" size="62" onKeyDown="countChars(document.post.aiosp_title,document.post.lengthT)" onKeyUp="countChars(document.post.aiosp_title,document.post.lengthT)"/><br />
			...
			<?php _e(' characters. Most search engines use a maximum of 60 chars for the title.', 'truethemes_localize') ?>
			</td>
		</tr>
