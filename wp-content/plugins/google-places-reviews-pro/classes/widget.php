...
class Google_Places_Reviews extends WP_Widget {
	...
	function add_gpr_widget_scripts() {
		...
		if ( defined( 'GPR_DEBUG' ) ) {

			wp_enqueue_script( 'grp_reviews_readmore', plugins_url( 'assets/js/gpr-readmore' . $suffix . '.js', dirname( __FILE__ ) ), array( 'jquery' ) );

			wp_enqueue_script( 'grp_reviews_main_scripts', plugins_url( 'assets/js/google-places-reviews' . $suffix . '.js', dirname( __FILE__ ) ), array( 'jquery' ) );
