...
class GPR_License {
	...
	function edd_wordimpress_license_admin_notices() {
		...
		if ( ! get_user_meta( $user_id, $this->licence_key_setting . '_license_ignore_notice' ) && current_user_can( 'install_plugins' ) ) {
			...
			if ( $status == 'invalid' || empty( $status ) ) {
				...
				$settings_link = '<a href="options-general.php?page=googleplacesreviews">' . __( 'activate your license', 'gpr' ) . '</a>';
				$hide_notice   = '<a href="?' . http_build_query( array_merge( $params, array( $this->licence_key_setting . '_license_ignore_notice' => '0' ) ) ) . '" rel="nofollow"> ' . __( 'Hide Notice', 'gpr' ) . '</a>';
	...
	function register_licence_assets( $hook ) {
		...
		if ( $hook == $this->settings_page ) {
			...
			wp_register_script( 'wordimpress_licencing_js', GPR_PLUGIN_URL . '/inc/licence/assets/js/licence.js' );
			...
			//CSS
			wp_register_style( 'wordimpress_licencing_css', GPR_PLUGIN_URL . '/inc/licence/assets/css/licence.css' );
