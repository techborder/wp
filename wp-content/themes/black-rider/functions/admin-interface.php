...
function inkthemes_optionsframework_add_admin() {
    ...
    $themename = blcr_get_option('of_themename');
    $shortname = blcr_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = blcr_get_option('of_template');
...
function inkthemes_optionsframework_options_page() {
    ...
    <div class="trail-notify">
        <h1><?php _e('Get Black Rider Theme PRO!','blcr'); ?></h1>
        <p><?php _e('You are Using the Lite Version of Black Rider Theme. Upgrade to Pro for extra features like Home Page Slider, Contact Page Template,Lead Capture form, FullWidth Page Template, Multiple Color Options and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/blackriders-wordpress-theme/" target="blank">Upgrade to Black Rider Theme PRO Now &rarr;','blcr'); ?></a>
    </div>
    <div class="wrap" id="of_container">
        <div id="of-popup-save" class="of-save-popup">
            <div class="of-save-save"><?php _e('Options Updated','blcr'); ?></div>
        </div>
        <div id="of-popup-reset" class="of-save-popup">
            <div class="of-save-reset"><?php _e('Options Reset','blcr'); ?></div>
        </div>
        <form action="" enctype="multipart/form-data" id="ofform">
            <div id="header">
                <div class="logo">
                    <h2><?php echo $themename; ?><?php _e('Options','blcr'); ?></h2>
                </div>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = blcr_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = blcr_get_option($option_id);
                }
                ?>
...
function inkthemes_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        blcr_update_option($clickedID, $uploaded_file['url']);
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            blcr_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    blcr_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    blcr_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            blcr_update_option($multicheck_id, 'false');
                        } else {
                            blcr_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    blcr_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    blcr_update_option($id, $border_array);
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'select':
                ...
                $select_value = blcr_get_option($value['id']);
            ...
            case "color":
                if(isset($value['std']))
                $val = $value['std'];
                $stored = blcr_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = blcr_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = blcr_get_option($value['id']);
...
function inkthemes_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = blcr_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (blcr_get_option($id) != "") {
            $val = blcr_get_option($id);
        }
