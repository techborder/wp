...
?>
<?php
function butterbelly_optionsframework_add_admin() {
    global $query_string;
    
    $themename =  butterbelly_get_option('of_themename');      
    $shortname =  butterbelly_get_option('of_shortname'); 
   
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			$options =  butterbelly_get_option('of_template'); 
			butterbelly_reset_options($options,'optionsframework');
...
$of_page = add_theme_page($themename,'Theme Options','edit_theme_options', 'optionsframework', 'butterbelly_optionsframework_options_page', 'div');
	
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page", 'butterbelly_load_only');
} 
add_action('admin_menu', 'butterbelly_optionsframework_add_admin');
/*-----------------------------------------------------------------------------------*/
/* Options Framework Reset Function - of_reset_options */
/*-----------------------------------------------------------------------------------*/
function butterbelly_reset_options($options,$page = ''){
	global $wpdb;
	$count = 0;
	...
	if($page == 'optionsframework'){
		butterbelly_delete_option( 'of_options' );
	}
}
/*-----------------------------------------------------------------------------------*/
/* Build the Options Page - optionsframework_options_page */
/*-----------------------------------------------------------------------------------*/
function butterbelly_optionsframework_options_page(){
    $options =  butterbelly_get_option('of_template');      
    $themename =  butterbelly_get_option('of_themename');
?>
<div class="trail-notify">
        <h1>Get ButterBelly Theme PRO!</h1>
        <p style="font-size:15px; line-height: 20px;"><?php _e('You are using the Lite Version of ButterBelly Theme. Upgrade to Pro for extra features like Home Page Slider Contact Page, Gallery Features, FullWidth Page Templates, Multiple Color Options and much more.','butterbelly');?></p>
        <?php $pro_theme_url='http://www.inkthemes.com/wp-themes/butterbelly-business-wordpress-theme/';?>
        <a href="<?php esc_url($inkthemes_url); ?>" target="blank"><?php _e('Upgrade to ButterBelly PRO Theme here >>','butterbelly');?></a>
    </div>
<div class="wrap1" id="of_container">
  <div id="of-popup-save" class="of-save-popup">
    <div class="of-save-save"><?php _e('Options Updated','butterbelly');?></div>
  </div>
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset"><?php _e('Options Reset','butterbelly');?></div>
  </div>
  <form action="" enctype="multipart/form-data" id="ofform">
  <?php wp_nonce_field('theme-update-option'); ?>
    <div id="header">
      <div class="logo">
        <h2><?php echo $themename; ?><?php _e('Options','butterbelly'); ?> </h2>
      </div>
        <?php $inkthemes_site_url='http://www.inkthemes.com';?>
        <a href="<?php esc_url($inkthemes_site_url);?>" target="new">
    ...
    <?php 
        ...
        $return = butterbelly_optionsframework_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
...
/*-----------------------------------------------------------------------------------*/
function butterbelly_load_only() {
...
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - butterbelly_ajax_callback */
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_of_ajax_post_action', 'butterbelly_ajax_callback');
function butterbelly_ajax_callback() {
	...
	if($save_type == 'upload'){
		...
		$uploaded_file = wp_handle_upload($filename,$override);
				...
				butterbelly_update_option( $clickedID , $uploaded_file['url'] );
	...
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		...
		foreach($options as $option_array){
			$id = $option_array['id'];
			$old_value = butterbelly_get_option($id);
			$new_value = '';
			...
			if(isset($option_array['id'])) { // Non - Headings...
					...
					if ( is_array($type)){
						foreach($type as $array){
							if($array['type'] == 'text'){
								...
								butterbelly_update_option( $id, stripslashes($new_value));
							}
						}                 
					}
					elseif($new_value == '' && $type == 'checkbox'){ // Checkbox Save						
						butterbelly_update_option($id,'false');
					}
					elseif ($new_value == 'true' && $type == 'checkbox'){ // Checkbox Save						
						butterbelly_update_option($id,'true');
					}
					elseif($type == 'multicheck'){ // Multi Check Save						
						...
						foreach ($option_options as $options_id => $options_value){							
							...
							if(!isset($output[$multicheck_id])){
							  butterbelly_update_option($multicheck_id,'false');
							}
							else{
							   butterbelly_update_option($multicheck_id,'true'); 
							}
						}
					} 
					elseif($type == 'typography'){							
						...
						butterbelly_update_option($id,$typography_array);							
					}
					elseif($type == 'border'){							
						$border_array = array();							
						...
						butterbelly_update_option($id,$border_array);							
					}
...
/*-----------------------------------------------------------------------------------*/
function butterbelly_optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'text':
			$val = $value['std'];
			$std = butterbelly_get_option($value['id']);
			if ( $std != "") { $val = $std; }
		...
		break;                
			...
			$select_value = butterbelly_get_option($value['id']);			 
		...
		case "upload":
			$value['std'] = '';
			if (isset($value['std'])){
			$output .= butterbelly_optionsframework_uploader_function($value['id'],$value['std'],null);
			}
		break;
		case "upload_min":
			
			$output .= butterbelly_optionsframework_uploader_function($value['id'],$value['std'],'min');
			
		break;
		case "color":
			$val = $value['std'];
			$stored  = butterbelly_get_option( $value['id'] );
		...
		case "typography":
			...
			$default = $value['std'];
			$typography_stored = butterbelly_get_option($value['id']);
		...
		case "border":
			...
			$default = $value['std'];
			$border_stored = butterbelly_get_option( $value['id'] );
		...
		if ( is_array($value['type'])) {
			foreach($value['type'] as $array){
					...
					$id = $array['id']; 
					$std = $array['std'];
					$saved_std = butterbelly_get_option($id);
...
/*-----------------------------------------------------------------------------------*/
/* OptionsFramework Uploader - butterbelly_optionsframework_uploader_function */
/*-----------------------------------------------------------------------------------*/
function butterbelly_optionsframework_uploader_function($id,$std,$mod){
	...
	$uploader = '';
    $upload = butterbelly_get_option($id);
	
	if($mod != 'min') { 
			$val = $std;
            if ( butterbelly_get_option( $id ) != "") { $val = butterbelly_get_option($id); }
